...
def plot_episodes_rewards_averaged(results):
    ...
    plt.plot(x, avg, label=f"{results['title']}")
...
def plot_episodes_length(title, learning_results):
    ...
    for results in learning_results:
        ...
        trend = np.poly1d(np.polyfit(episode, iterations, 1))
        plt.plot(episode, trend(episode), color='k', linewidth=0.8)
    plt.legend(loc="upper right")
    plt.show()


def plot_averaged_episode_rewards(title, learning_results):  # cherry
    """Plot length of every episode"""
    plt.figure(figsize=(10, 8))
    plt.ylabel('averaged step rewards', fontsize=20)
    ...
    for results in learning_results:
        ...
        plt.plot(episode, cumrewards / (iterations), label=f"{results['title']}")
    plt.legend(loc="upper right")
    # plt.savefig('./results/averaged_episode_rewards.pdf', bbox_inches='tight')
    plt.show()
...
def plot_averaged_cummulative_rewards(title, all_runs):
    ...
    new_plot(title)
    for r in all_runs:
        plot_episodes_rewards_averaged(r)  # cherry
    plt.legend(loc="lower right")
    plt.savefig('./results/averaged.pdf', bbox_inches='tight')
...
class PlotTraining:
    ...
    def plot_durations(self, average_window=2):
        plt.figure()
